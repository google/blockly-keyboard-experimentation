...
export class Clipboard {
  ...
  private registerCutContextMenuAction() {
    const cutAction: ContextMenuRegistry.RegistryItem = {
      displayText: (scope) =>
        `Cut (${getShortActionShortcut(Constants.SHORTCUT_NAMES.CUT)})`,

      // This is similar to the preconditionFn for the keyboard shortcut in core,
      // but it doesn't have the check if a gesture is in progress.
  ...
  private registerCopyContextMenuAction() {
    const copyAction: ContextMenuRegistry.RegistryItem = {
      displayText: (scope) =>
        `Copy (${getShortActionShortcut(Constants.SHORTCUT_NAMES.COPY)})`,
      // This is similar to the preconditionFn for the keyboard shortcut in core,
      // but it doesn't have the check if a gesture is in progress.
      preconditionFn: (scope) => {
        const focused = scope.focusedNode;
  ...
  private registerPasteContextMenuAction() {
    const pasteAction: ContextMenuRegistry.RegistryItem = {
      displayText: (scope) =>
        `Paste (${getShortActionShortcut(Constants.SHORTCUT_NAMES.PASTE)})`,
